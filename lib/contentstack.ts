...
export const getEntries = async <T>(
  contentType: string,
  options?: { includeReference?: string[]; queryBuilder?: QueryBuilder }
): Promise<T[]> => {
  ...
  const result = await entryRequest.find();
  return (result.entries || []) as T[];
};
...
export const getEntry = async <T>(
  contentType: string,
  entryUid: string,
  options?: { includeReference?: string[] }
): Promise<T | null> => {
 ...
 */
export const getAllShoes = async (): Promise<ContentstackShoe[]> => {
 ...
 */
export const getShoeByUrl = async (shoeUrl: string): Promise<ContentstackShoe | null> => {
 ...
 */
export const getHomepage = async (): Promise<ContentstackHomepage | null> => {
  try {
